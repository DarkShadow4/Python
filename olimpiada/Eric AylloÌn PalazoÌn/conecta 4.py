...
def comprobar_fila(fila, jugador):
    """Funcion que comprueba si alguien ha conectado una columna"""
    i = 0
    while i < len(fila):
        if fila[i:i+4:] == [jugador, jugador, jugador, jugador]: #puesto que fila[i:i+4:] es una lista, la comparo con la lista que debe ser para que haya ganado el jugador
            return "Jugador {0}. Has ganado!!".format(jugador)
        i += 1
    return "Sigue"

def comprobar_columna(columna, jugador):
    """Funcion que comprueba si alguien ha conectado una columna"""
    i = 0
    while i < len(columna):
        if columna[i:i+4:] == [jugador, jugador, jugador, jugador]:
            return "Jugador {0}. Has ganado!!".format(jugador)
        i += 1
    return "Sigue"

def comprobar_diagonal(tablero, jugador):
    """Funcion que comprueba si alguien ha conectado una diagonal"""
    for y in range(len(tablero)):
        for x in range(len(tablero)):
            if y+3 < len(tablero):
                if x+3 < len(tablero):
                    if [tablero[y][x], tablero[y+1][x+1], tablero[y+2][x+2], tablero[y+3][x+3]] == [jugador, jugador, jugador, jugador]: #comparo una lista de longitud 4 y valores correspondientes
                                                                                                                # con la diagonal con la lista que debe ser para que haya ganado
                                                                                                                # el jugador
                        return "Jugador {0}. Has ganado!!".format(jugador)
            if y+3 < len(tablero):
                if x-3 >= 0:
                    if [tablero[y][x], tablero[y+1][x-1], tablero[y+2][x-2], tablero[y+3][x-3]] == [jugador, jugador, jugador, jugador]:
                        return "Jugador {0}. Has ganado".format(jugador)
    return "Sigue"

def comprobar_estado_partida(tablero, jugador):
    """Funcion que comprueba el estado de la partida para ver si ha ganado alguien"""
    for fila in tablero:
        resultado = comprobar_fila(fila, jugador)
        if resultado != "Sigue":
            return resultado
    for i in range(len(tablero)):
        columna = get_columna(tablero, i)
        resultado = comprobar_columna(columna, jugador)
        if resultado != "Sigue":
            return resultado
    # implementar diagonal
    resultado = comprobar_diagonal(tablero, jugador)
...
while end == False:
    if njugador%2 == 0:
        ...
        jugador = 2
    valida = jugada(tablero, jugador, n)
    if valida:
        resultado = comprobar_estado_partida(tablero, jugador)
        if resultado == "Sigue":
            njugador += 1
            if njugador == n**2:
                end = True
